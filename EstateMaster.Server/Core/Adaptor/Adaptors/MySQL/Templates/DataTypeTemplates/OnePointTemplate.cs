...
namespace EstateMaster.Server.Adaptor.Adaptors.MySQL.Templates.DataTypeTemplates
{
    ...
    public class OnePointTemplate : DataDefination, IDataTypeTemplate
    {

        private string template = "[STRUCTURE]([LIMIT])";

        private ColumnItem column { get; set; }

        public OnePointTemplate(ColumnItem column)
        {
            this.column = column;
        }
        ...
        protected override string ToSQLBase()
        {
            return template
                ...
                .Replace("[LIMIT]", column.maxLength.ToString())
                .Trim();
        }
    }
